...
class AppDelegate: UIResponder, UIApplicationDelegate, UrcaInfeed {
    ...
    var window: UIWindow?
    var navigationCntroller: UINavigationController?

    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        // Override point for customization after application launch.
        let top = UIStoryboard(name: "Main", bundle: nil).instantiateViewControllerWithIdentifier("\(ViewController.self)")
        navigationCntroller = UINavigationController(rootViewController: top)
        navigationCntroller?.setNavigationBarHidden(false, animated: true)
        navigationCntroller?.setToolbarHidden(true, animated: false)
        window?.rootViewController = navigationCntroller
        window?.addSubview(navigationCntroller!.view)
